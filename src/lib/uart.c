...
typedef struct UART_INSTANCE_STRUCT
{
	...
	HOSTADEF(UART_PL011) * const ptArea;
#else
...
static const UART_INSTANCE_T atUartInstances[] =
{
	...
	{
		(NX9X2_MPW_UART_PL011_AREA_T * const)Addr_NX9X2_MPW_secenc_uart
	}

#elif ASIC_TYP==ASIC_TYP_NETX4000_RELAXED || ASIC_TYP==ASIC_TYP_NETX4000
	{
		(HOSTADEF(UART) * const)HOSTADDR(uart0),
		HOSTMMIO(UART0_RXD),
		HOSTMMIO(UART0_TXD),
		HOSTMMIO(UART0_RTSN),
		HOSTMMIO(UART0_CTSN)
	...
	{
		(HOSTADEF(UART) * const)HOSTADDR(uart1),
		HOSTMMIO(UART1_RXD),
		HOSTMMIO(UART1_TXD),
		HOSTMMIO(UART1_RTSN),
		HOSTMMIO(UART1_CTSN)
	...
	{
		(HOSTADEF(UART) * const)HOSTADDR(uart2),
		HOSTMMIO(UART2_RXD),
		HOSTMMIO(UART2_TXD),
		HOSTMMIO(UART2_RTSN),
		HOSTMMIO(UART2_CTSN)
	...
	{
		(NX90MPW_UART_AREA_T * const)Addr_NX90MPW_uart_com
	}
